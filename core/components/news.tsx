...
function getPostCategory(
  title: string,
  content: string,
): "OPERATIONS" | "COMMUNITY" | "SUSTAINABILITY" {
...
export default function News() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const pageControls = rootRef.current?.querySelectorAll(
        "[data-page-controls]",
      );
  ...
  const currentArticles: NewsArticle[] = (postsData?.posts ?? []).map(
    (post, idx) => ({
    ...
    >
      ...
      <section ref={introRef} className="bg-[#f4f4f4] py-16 sm:py-24">
        <div className="mx-auto max-w-[980px] px-5">
          ...
          >
            Stay Updated with{" "}
            <span className="italic text-[#14874f]">NEPN</span>
          </h2>
          ...
          >
            {isLoading ? (
              ...
              currentArticles.map((article, index) => {
                ...
                const titleColor = article.isGreenTitle
                  ? "text-[#14874f]"
                  : "text-[#1e2620]";
                    ...
                    <div className="p-6 flex-1 flex flex-col justify-between">
                      <div>
                        <div className="flex mb-4">
                          <span
                            className={`text-[8.5px] font-bold text-white px-2 py-0.5 tracking-wider rounded uppercase ${categoryBg}`}
                          >
                            {article.category}
                          </span>
                        </div>
