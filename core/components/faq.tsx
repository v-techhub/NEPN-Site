...
function AccordionItem({
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      gsap.fromTo(body, { height: 0, opacity: 0 }, { height: h, opacity: 1, duration: 0.42, ease: "power3.out" });
      gsap.to(iconRef.current, { rotation: 45, duration: 0.3, ease: "power2.out" });
    } else {
      gsap.to(body, { height: 0, opacity: 0, duration: 0.32, ease: "power3.inOut" });
      gsap.to(iconRef.current, { rotation: 0, duration: 0.3, ease: "power2.out" });
    }
  }, [isOpen]);

  return (
    <div ref={itemRef} className="border-b last:border-b-0" style={{ borderColor: "rgba(0,0,0,0.1)" }}>
      <button onClick={onToggle} className="w-full flex items-center justify-between gap-4 py-5 text-left group">
        ...
        <div
          ...
          aria-hidden
        >
          <svg className="w-3 h-3" fill="none" stroke={isOpen ? "#fff" : "#006633"} strokeWidth={2.5} viewBox="0 0 24 24">
      ...
      <div ref={bodyRef} className="overflow-hidden" style={{ height: 0, opacity: 0 }}>
        <div ref={innerRef} className="pb-5 pr-12">
          <p className="text-[14.5px] leading-relaxed" style={{ color: "#555", fontFamily: "'Barlow', sans-serif" }}>
            {faq.answer}
          </p>
        </div>
      </div>
...
export default function FAQ() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(leftRef.current, { x: -50, opacity: 0 }, { x: 0, opacity: 1, duration: 0.85, ease: "power3.out", scrollTrigger: st });
      gsap.fromTo(
        [eyebrowRef.current, headingRef.current, subRef.current, cardRef.current],
        { y: 28, opacity: 0 },
        { y: 0, opacity: 1, duration: 0.65, ease: "power3.out", stagger: 0.1, delay: 0.15, scrollTrigger: st },
      );
      gsap.fromTo(rightRef.current, { x: 50, opacity: 0 }, { x: 0, opacity: 1, duration: 0.85, ease: "power3.out", scrollTrigger: st });

      const activeFAQRows = itemRefs.current.filter(Boolean);
      if (activeFAQRows.length > 0) {
        gsap.fromTo(activeFAQRows, { y: 24, opacity: 0 }, { y: 0, opacity: 1, duration: 0.55, ease: "power3.out", stagger: 0.09, delay: 0.3, scrollTrigger: st });
      }
    }, sectionRef);
  ...
  return (
    <section ref={sectionRef} className="w-full py-16 lg:py-24" style={{ background: "#f0ede6", fontFamily: "'Barlow', sans-serif" }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[420px_1fr] gap-12 lg:gap-20 xl:gap-24">
          <div ref={leftRef} className="flex flex-col">
            <div ref={eyebrowRef} className="flex items-center gap-3 mb-4">
              ...
              <span className="text-[10.5px] font-bold tracking-[0.24em] uppercase text-gray-500" style={{ fontFamily: "'Barlow Condensed', sans-serif" }}>
                FAQ
              </span>
            </div>
            ...
            <p ref={subRef} className="text-[15px] leading-relaxed mb-8" style={{ color: "#666", maxWidth: "360px" }}>
              Quick answers to common enquiries about our operations, partnerships, and more.
            </p>

            <div ref={cardRef} className="flex flex-col p-8 lg:p-9 mt-auto" style={{ background: "#006633" }}>
              <h3
              ...
              <p className="text-[14px] leading-relaxed mb-7" style={{ color: "rgba(255,255,255,0.72)" }}>
                Our team is ready to help with any enquiry about operations, investment opportunities, or community programmes.
              </p>
              <button
            ...
            {isLoading ? (
              <div className="space-y-4">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="py-5 border-b border-black/10 animate-pulse flex justify-between items-center">
              ...
              <div className="py-16 text-center">
                <p className="text-neutral-400 font-medium text-sm">FAQ content has not been published yet.</p>
              </div>
            ) : (
              faqs.map((faq, i) => (
                <AccordionItem
                  ...
                  onToggle={() => setOpenId(activeOpenId === faq.id ? null : faq.id)}
                  itemRef={(el) => {
