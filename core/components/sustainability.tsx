...
export default function Sustainability() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const commitCopy =
        rootRef.current?.querySelectorAll("[data-commit-copy]");
      const ehsHeader = rootRef.current?.querySelectorAll("[data-ehs-header]");
      const ehsGrid = rootRef.current?.querySelectorAll(
        "[data-ehs-grid] > div",
      );
      const safetyCopy =
        rootRef.current?.querySelectorAll("[data-safety-copy]");
      const safetyCard =
        rootRef.current?.querySelectorAll("[data-safety-card]");
      const csrHeader = rootRef.current?.querySelectorAll("[data-csr-header]");
      const csrGrid = rootRef.current?.querySelectorAll(
        "[data-csr-grid] > div",
      );
      const impactCopy =
        rootRef.current?.querySelectorAll("[data-impact-copy]");
      const statsBar = rootRef.current?.querySelectorAll(
        "[data-stats-bar] > div",
      );
      ...
      if (ehsHeader?.length) {
        animateReveal(
          ehsHeader,
          rootRef.current?.querySelector("[data-ehs-trigger]"),
        );
      }
      ...
      if (safetyCopy?.length) {
        animateReveal(
          safetyCopy,
          rootRef.current?.querySelector("[data-safety-trigger]"),
        );
      }
      ...
      if (csrHeader?.length) {
        animateReveal(
          csrHeader,
          rootRef.current?.querySelector("[data-csr-trigger]"),
        );
      }
      ...
      if (impactCopy?.length) {
        animateReveal(
          impactCopy,
          rootRef.current?.querySelector("[data-impact-trigger]"),
        );
      }
      ...
      if (statsBar?.length) {
        animateReveal(
          statsBar,
          rootRef.current?.querySelector("[data-stats-trigger]"),
        );
      }
    }, rootRef);
  ...
  return (
    ...
    >
      ...
      <section ref={commitmentRef} className="bg-[#f4f4f4] py-16 sm:py-24">
        <div className="mx-auto max-w-[980px] px-5 text-center">
          ...
          >
            Building a{" "}
            <span className="italic text-[#14874f]">Sustainable Future</span>
          </h2>
          ...
          >
            ...
            supply, job creation, infrastructure development, and government
            revenue. Its shared value approach ensures benefits for employees,
            partners, host communities, and society, while promoting high
            industry standards. The Company emphasizes transparency and
            accountability, continuously improving its sustainability
            performance and aligning its reporting with global standards such as
            the Global Reporting Initiative (GRI) and International Finance
            Corporation (IFC).
          </p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[980px] px-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 text-[#5d6763] text-sm leading-[1.8] font-sans mb-12">
            <p data-ehs-header>
              ...
              environmental management. It successfully obtained Environmental
              Impact Assessment (EIA) approval for the Qua Ibo Field Development
              Project from the Department of Petroleum Resources (DPR) and the
              Federal Ministry of Environment (FMENV), based on a study
              conducted by the University of Uyo Consultancy Limited. To
              minimize environmental and community impact, all development wells
              are directionally drilled from a single location situated away
              from villages, while production facilities are placed
              approximately 800 meters from the wellheads. Even as future
              operations may require additional drilling locations for optimal
              reservoir performance, environmental protection and community
              safety remain key priorities.
            </p>
            <p data-ehs-header>
              NEPN operates a structured HSE Management System that identifies
              hazards, evaluates and prioritizes risks, and implements
              mitigation measures, with continuous monitoring and improvement
              built into all operations. This system protects employees,
              contractors, equipment, host communities, the public, and the
              environment, while ensuring compliance with industry standards
              across facility design, asset lifecycle, and operational
              activities. Its effectiveness is reflected in an outstanding
              safety record, including zero Lost Time Accidents during the NRG
              201 rig operations (2008–2009), 700,000 man-hours without incident
              during Henan drilling operations in 2013, and over 3.3 million
              cumulative man-hours without a Lost Time Incident, demonstrating a
              sustained commitment to safety, operational excellence, and
              responsible energy development.
            </p>
          </div>

          <div data-ehs-grid className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {EHS_CARDS.map((card) => (
              <div
                key={card.title}
              ...
              >
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  ...
                  <p className="text-sm font-bold text-[#1b1cff]">
                    {card.metric}
                  </p>
                </div>
              </div>
        ...
        <div className="mx-auto max-w-[980px] px-5">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.1fr] gap-12 lg:gap-16 items-center">
            ...
            <div>
              ...
              <ul
                data-safety-copy
                className="space-y-4 font-sans text-xs sm:text-sm text-[#4a5550]"
              >
        ...
        <div className="mx-auto max-w-[980px] px-5">
          <div className="text-center mb-12 sm:mb-16">
            ...
            >
              ...
              Through our Corporate Social Responsibility (CSR) initiatives, we
              support community development, education, healthcare,
              environmental sustainability, and local empowerment. We strive to
              operate with integrity, prioritize safety, and build lasting
              partnerships that contribute to sustainable growth and improved
              livelihoods in the communities where we operate.
            </p>
          </div>

          {/* CSR Image cards grid */}
          <div data-csr-grid className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {isLoading ? (
              <>
                <CsrSkeletonCard />
                <CsrSkeletonCard />
              ...
              currentCsrCards.map((card, index) => (
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-6 flex flex-col justify-end translate-y-2 group-hover:translate-y-0 transition-transform duration-500 ease-out">
                    <p className="text-[11px] sm:text-[12px] text-white leading-relaxed font-sans max-w-[95%] opacity-90 group-hover:opacity-100 transition-opacity duration-500 font-semibold">
                      {card.text}
                    </p>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.2fr] gap-12 lg:gap-16 items-center">
            ...
            <div>
              ...
              <p
                ...
                className="text-[#5d6763] text-sm sm:text-[14.5px] leading-relaxed mb-8 font-sans"
              >
                We track and report our sustainability performance across
                environmental, social, and governance metrics to ensure
              ...
              >
                {PERFORMANCE_METRICS.map((metric) => (
                  <div
                    key={metric.label}
                    className="border-l-[3.5px] border-[#14874f] pl-4"
                  >
                    <p
      ...
      <section
        data-stats-trigger
        className="bg-white border-t border-b border-neutral-200/60 py-10 sm:py-12"
      >
        <div className="mx-auto max-w-[980px] px-5">
          <div
            data-stats-bar
            className="grid grid-cols-2 md:grid-cols-4 gap-y-8 gap-x-4 text-center divide-y md:divide-y-0 md:divide-x divide-neutral-200/60"
          >
            <div className="pt-0 md:pt-0">
              <p
                className="text-[34px] sm:text-[40px] font-black text-[#14874f] leading-none"
                style={{ fontFamily: "'Clash Display', sans-serif" }}
              >
                100%
              </p>
              <p
                className="mt-2 text-[9px] font-bold text-[#14874f] uppercase tracking-widest leading-none"
                style={{ fontFamily: "'Clash Display', sans-serif" }}
              >
                Nigerian Owned
              </p>
            ...
            <div className="pt-0 md:pt-0">
              <p
                className="text-[34px] sm:text-[40px] font-black text-[#ef3b3b] leading-none"
                style={{ fontFamily: "'Clash Display', sans-serif" }}
              >
                Zero
              </p>
              <p
                className="mt-2 text-[9px] font-bold text-[#ef3b3b] uppercase tracking-widest leading-none"
                style={{ fontFamily: "'Clash Display', sans-serif" }}
              >
                Major HSE Incidents
              </p>
            ...
            <div className="pt-6 md:pt-0">
              <p
                className="text-[34px] sm:text-[40px] font-black text-[#1b1cff] leading-none"
                style={{ fontFamily: "'Clash Display', sans-serif" }}
              >
                2040
              </p>
              <p
                className="mt-2 text-[9px] font-bold text-[#1b1cff] uppercase tracking-widest leading-none"
                style={{ fontFamily: "'Clash Display', sans-serif" }}
              >
